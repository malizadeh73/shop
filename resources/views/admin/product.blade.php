            ...
            <div class="row">
                <div class="col-xs-12">
                    <a href="/admin/product/new">
                        <input type="button" class="btn btn-primary pull-right" value="افزودن محصول">
                    </a>
                </div>
            </div>
            <div class="card">
                <div class="card-header" data-background-color="purple">
                    <h4 class="title">مدیریت محتوی محصولات</h4>
                    <p class="category">لیست محصولات موجود در سایت</p>
                </div>
                    ...
                    <div class="col-md-10 col-md-offset-1 col-xs-12">
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-md-10 pull-right">
                                <input name="search_product" class="form-control">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-xs-12">
                                    ...
                                    <div class="table-responsive">
                                        <table id="" class="table">
                                            ...
                                            <tr>
                                                ...
                                                <th class="col-xs-3 text-right">عنوان</th>
                                                <th class="col-xs-2 text-right">کد</th>
                                                <th class="col-xs-2 text-right">قیمت</th>
                                                <th class="col-xs-2 text-right">تخفیف</th>
                                            ...
                                            @foreach($product as $one_product)
                                                <tr data-status="">
                                                    <td>{{$counter_product = $counter_product +1}}</td>
                                                    <td data-id="{{$one_product->id}}" class="">{{$one_product->name}}</td>
                                                    <td class="">{{$one_product->code}}</td>
                                                    <td class="">{{$one_product->price}}</td>
                                                    <td class="">{{$one_product->off}}</td>
                                                    <td class="actional">
                                                        <span data-id="{{$one_product->id}}" data-title="delete_product"
                                                              class="fa fa-trash delete_product_button"
                                                              data-toggle="tooltip" title="حذف"></span>
                                                        &nbsp;
                                                        <a href="/admin/product/edit/{{$one_product->name}}"
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            paginate(".pagination", all_page, page_now, 7, "/admin/product/list");
            $('[data-toggle="tooltip"]').tooltip();
            $("span[data-title='delete_product']").click(function (e) {
                var $tr = $(this).closest('tr');
                var data_id = $(this).attr('data-id');
                swal({
                        title: "آیا شما مطمئن هستید؟",
                        text: "در صورت حذف محصول هیچ راه بازگشتی نخواهد بود!",
                        type: "warning",
            ...
            $("input[name='search_product']").keyup(function (event) {
                        ...
                        success: function (response) {
                            ...
                            if (response['status']) {
                                ...
                                for (var i = 0; i < response['data'].length; i++) {
                                    ...
                                    html_string += '<span data-id="'+ response['data'][i].id+'" data-title="delete_product" class="flaticon-trash-2 delete_student_button" data-toggle="tooltip" title="حذف"></span>';
                                    html_string += ' &#160;';
                                    html_string += '<a href="/admin/product/edit/'+ response['data'][i].title+'" data-toggle="tooltip" title="ویرایش">';
